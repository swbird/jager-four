...
async function main() {
    try {
        ...
        await fs.writeFile('claim_results.txt', '地址,领取结果,交易哈希\n');
        ...
        for (let i = 0; i < addrs.length; i++) {
            ...
            try {
                ...
                await fs.appendFile('claim_results.txt', `${address},${result}\n`);
                
                // 等待3秒再处理下一个地址
                await new Promise(resolve => setTimeout(resolve, 3000));
            } catch (error) {
                console.error(`处理地址 ${address} 时出错:`, error);
                await fs.appendFile('claim_results.txt', `${address},失败,${error.message}\n`);
            }
        }
